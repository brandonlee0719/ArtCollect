...
const Profile = () => {
    ...
    const handleSubmitForm = async (data) => {
        ...
        axios.put(requestURL, data)
            .then(response => console.log("-000000000000"))
            .catch(error => {
    ...
    const handleSubmitProfilePicture = async (file, field) => {
        ...
        form_Data.append('ref', 'author'); // link the image to a content type
        form_Data.append('refId', authorId); // link the image to a specific entry
        form_Data.append('field', field); // link the image to a specific field
console.log(`Bearer ${jwt}`,  `${api.baseUrl}/api/upload`)
        await axios({
            method: 'post',
